...
class GT_Spirit_License_Key {
	...
	static function setup() {
		...
		// Define Product ID.
		define( 'GT_SPIRIT_PRODUCT_ID', 171494 );

		// Define Update API URL.
		define( 'GT_SPIRIT_STORE_API_URL', 'https://themezee.com' );
	...
	static function activate_license() {
		...
		// Call the custom API.
		$response = wp_remote_post( GT_SPIRIT_STORE_API_URL, array( 'timeout' => 35, 'sslverify' => true, 'body' => $api_params ) );
		...
		update_option( 'gt_spirit_theme_options', $theme_options );

		delete_transient( 'gt_spirit_license_check' );
	...
	static function check_license() {
		...
		$status = get_transient( 'gt_spirit_license_check' );
		...
		if ( false === $status ) {
			...
			if ( '' !== $license_key and 'inactive' !== $theme_options['license_status'] ) {
				...
				// Call the custom API.
				$response = wp_remote_post( GT_SPIRIT_STORE_API_URL, array( 'timeout' => 25, 'sslverify' => true, 'body' => $api_params ) );
			...
			set_transient( 'gt_spirit_license_check', $status, DAY_IN_SECONDS );
		}
