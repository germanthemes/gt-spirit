...
class GT_Spirit_Custom_Fonts {
	...
	static function remove_default_theme_fonts() {
		...
		// Remove default Quicksand font if not needed or GT Local Font plugin already loads it.
		if ( ( ! in_array( 'Quicksand', $selected_fonts ) && ! is_customize_preview() ) or class_exists( 'GermanThemes_Local_Fonts' ) ) {
			wp_dequeue_style( 'gt-spirit-theme-fonts' );
		}
	}
	...
	static function get_available_fonts() {
		...
		$fonts = array(
			...
			'Quicksand'                   => 'Quicksand', // default font from /assets/css/theme-fonts.css
