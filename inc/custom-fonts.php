...
class GT_Spirit_Custom_Fonts {
	...
	static function setup() {
		...
		add_filter( 'gt_spirit_get_fonts_url', array( __CLASS__, 'get_custom_fonts_url' ) );
	}
	...
	static function add_custom_fonts_in_frontend() {
		wp_add_inline_style( 'gt-spirit-stylesheet', self::get_custom_fonts_css() );
	}
	...
	static function add_custom_fonts_in_editor() {
		wp_add_inline_style( 'gt-spirit-editor-styles', self::get_custom_fonts_css() );
	}
	...
	static function get_custom_fonts_css() {
		...
		if ( $theme_options['text_font'] !== $default_options['text_font'] ) {
			$font_variables .= '--gt-spirit--text-font: ' . self::get_font_family( $theme_options['text_font'] );
		}

		// Set Title Font.
		if ( $theme_options['title_font'] !== $default_options['title_font'] ) {
			$font_variables .= '--gt-spirit--title-font: ' . self::get_font_family( $theme_options['title_font'] );
		}

		// Set Title Font Weight.
		if ( $theme_options['title_is_bold'] !== $default_options['title_is_bold'] ) {
			$font_variables .= '--gt-spirit--title-font-weight: ' . ( $theme_options['title_is_bold'] ? 'bold' : 'normal' ) . '; ';
		}

		// Set Title Text Transform.
		if ( $theme_options['title_is_uppercase'] !== $default_options['title_is_uppercase'] ) {
			$font_variables .= '--gt-spirit--title-text-transform: ' . ( $theme_options['title_is_uppercase'] ? 'uppercase' : 'none' ) . '; ';
		}

		// Set Navi Font.
		if ( $theme_options['navi_font'] !== $default_options['navi_font'] ) {
			$font_variables .= '--gt-spirit--navi-font: ' . self::get_font_family( $theme_options['navi_font'] );
		}

		// Set Navi Font Weight.
		if ( $theme_options['navi_is_bold'] !== $default_options['navi_is_bold'] ) {
			$font_variables .= '--gt-spirit--navi-font-weight: ' . ( $theme_options['navi_is_bold'] ? 'bold' : 'normal' ) . '; ';
		}

		// Set Navi Text Transform.
		if ( $theme_options['navi_is_uppercase'] !== $default_options['navi_is_uppercase'] ) {
			$font_variables .= '--gt-spirit--navi-text-transform: ' . ( $theme_options['navi_is_uppercase'] ? 'uppercase' : 'none' ) . '; ';
		}
